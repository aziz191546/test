    ...
    try {
      ...
      if (temptype == "toggle") {
        try {
          ...
          return message.reply({embed: new Discord.MessageEmbed()
            .setTitle(`✅ ${client.settings.get(message.guild.id, "antidiscord").enabled ? "**Enabled** antidiscords": "**Disabled** Anti Discord Links"}`)
            .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
          ...
          .then(collected => {
            ...
            if (channel) {
              ...
              if (antisettings.includes(channel.id)) return message.reply({embed: new Discord.MessageEmbed()
                .setTitle(`❌ ERROR | The Channel: \`${channel.name}\` is already added to the Anti Discord Links Whitelist`)
                .setColor(es.wrongcolor)
                .setFooter(es.footertext, es.footericon)
              });
              try {
                ...
                return message.reply({embed: new Discord.MessageEmbed()
                  .setTitle(`✅ The Channel: \`${channel.name}\` is now registered as an Whitelisted Anti Discord Links Channel`)
                  .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
        ...
        await tempmsg.channel.awaitMessages(m => m.author.id === message.author.id, {
          ...
          .then(collected => {
            ...
            if (channel) {
              let antisettings = client.settings.get(message.guild.id, "antidiscord.whitelistedchannels")
              if (!antisettings.includes(channel.id)) return message.reply({embed: new Discord.MessageEmbed()
                .setTitle(`❌ ERROR | The Channel: \`${channel.name}\` is not added to the Anti Discord Links Whitelist yet`)
                .setColor(es.wrongcolor)
                .setFooter(es.footertext, es.footericon)
              });
              try {
                ...
                return message.reply({embed: new Discord.MessageEmbed()
                  .setTitle(`✅ The Channel: \`${channel.name}\` is now **NOT** registered as an Whitelisted Anti Discord Links Channel anymore`)
                  .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
                  .setDescription(`Every single Channel:\n<#${client.settings.get(message.guild.id, "antidiscord.whitelistedchannels").join(">\n<#")}>\nis not a checked by the Anti Discord Links System`.substr(0, 2048))
                  .setFooter(es.footertext, es.footericon)
                });
