    ...
    try {
      ...
      if(!member.roles.cache.some(r=>r.id == (role.id)))
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`❌ I cannot remove that Role from this Member, because he doesn't have it!`)
        );
      var ge = false;
      ...
      message.channel.send(new MessageEmbed()
        ...
        .setFooter(es.footertext, es.footericon)
        .setTitle(`✅ removed \`${role.name}\` from \`${member.user.tag}\``)
        .setDescription("If the role not removed a cause he is higher/Equal to my role Position or already has it!")
      );
