    ...
    try {
      ...
      if (!warnmember)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`❌ Please add a Member you want to unwarn!`)
      ...
      if (!args[1])
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`❌ Please add the Warning you want to remove from him`)
      ...
      if (moderationPosition <= memberPosition)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`❌ I cannot warn someone, who is above/equal you`)
        );
      ...
      try {
        ...
        message.channel.send(new MessageEmbed()
          .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`✅ Removed the Warn from ${warnmember.user.tag}`)
          .addField(`Warned by:`, `\`${warned_by}\``, true)
          .addField(`Warned at:`, `\`${warned_at}\``, true)
          .addField(`Warned in:`, `\`${warned_in}\``, true)
          .addField(`Warn Reason:`, `\`${warning.reason.length > 900 ? warning.reason.substr(0, 900) + ` ...` : warning.reason}\``, true)
