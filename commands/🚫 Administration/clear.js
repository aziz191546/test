    ...
    try {
      ...
      if(args[1]){
        if(args[1].toLowerCase() == "bots" || args[1].toLowerCase() == "bot"){
          ...
          if(args[0]){
              if(Number(args[0]) > 5000 || Number(args[0]) < 0) return message.reply("**❌ Maximum amount of Messages to be pruned are 5000 (minimum 1)**")
              if(isNaN(args[0])) return message.reply("**❌ Maximum amount of Messages to be pruned are 5000 (minimum 1)**")
