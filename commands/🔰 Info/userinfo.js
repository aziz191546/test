  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {   
      ...
      try{
        ...
        embeduserinfo.addField('**➡️ Username:**',`<@${user.id}>\n\`${user.tag}\``,true)
        embeduserinfo.addField('**➡️ ID:**',`\`${user.id}\``,true)
        embeduserinfo.addField('**➡️ Avatar:**',`[\`Link to avatar\`](${user.displayAvatarURL({ format: "png" })})`,true)
        embeduserinfo.addField('**➡️ Flags:**',`\`${userFlags.length ? userFlags.map(flag => flags[flag]).join(', ') : 'None'}\``,true)
        embeduserinfo.addField('**➡️ Status:**',`\`${statuses[user.presence.status]} ${user.presence.status}\``,true)
        embeduserinfo.addField('**➡️ Is a Bot:**',`\`${user.bot ? "✔️" : "❌"}\``,true)
        var userstatus = "Not having an activity";
        if(activity){
          if(activity.type === "CUSTOM_STATUS"){
            let emoji = `${activity.emoji ? activity.emoji.id ? `<${activity.emoji.animated ? "a": ""}:${activity.emoji.name}:${activity.emoji.id}>`: activity.emoji.name : ""}`
        ...
        embeduserinfo.addField('**➡️ Activity:**',`${userstatus}`)
