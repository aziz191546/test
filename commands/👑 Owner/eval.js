  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    if ("604041543485227009" !== message.author.id)
      return message.channel.send(new MessageEmbed()
        ...
        .setTitle(`${emoji.msg.ERROR}  Error | You are not allowed to run this command! Only the **\`[𝗔𝗭𝗜𝗭 𝗞𝗔𝗠𝗠𝗢𝗨𝗡]#1919\`** is allowed to run this Cmd`)
      );
    if (!args[0])
      return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor)
    ...
    try {
      ...
      let evalEmbed = new MessageEmbed()
        .setTitle(`💎BestBot💎 Development | Evaluation`)
        .setColor(es.color);
