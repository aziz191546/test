  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    if(!payamount)
      return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor)
        .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
        .setTitle(`❌ You didn't remove the payamount`)
        .setDescription(`Usage: \`${prefix}removemoney <@USER> <Amount>\`\n\n\Example: \`${prefix}removemoney <@604041543485227009> 1919\``)
      );
    ...
    return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor)
        .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
        .setTitle(`❌ You can't remove a negative Amount of Money or no Money, to ${topay}`)
      );
    ...
    if(payamount > data2.balance)
      return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor)
        .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
        .setTitle(`❌ You can't remove more Money than you he has in his/her **👛 Pocket (\`${data2.balance} 💸\`)**`)
      );
    ...
    return message.reply(new MessageEmbed()
      .setColor(es.color)
      .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
      .setTitle(`✅ **You removed \`${payamount} 💸\` to \`${topay.tag}\`**`)
