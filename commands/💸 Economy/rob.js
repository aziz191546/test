    ...
    try {
      ...
      if(!user)
        return message.channel.send(new MessageEmbed()
          ...
          .setTitle(`❌ You didn't pinged to whom you want to rob`)
      ...
      if(data.rob !== 0 && timeout - (Date.now() - data.rob) > 0){
        ...
        return message.reply({embed: new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(message.author.tag, message.author.displayAvatarURL({dynamic: true}))
          .setTitle(`❌ You've already robbed Today!!`)
          .setDescription(`Try again in ${time.map(i=> `\`${i}\``).join(", ")}\n\n👛 You still have \`${nFormatter(Math.floor(data.balance))} 💸\` in your Pocket`)
        });
      } 
      //YEA
      else {
        if(data2.balance < 500) return message.reply("❌ He does not have enough balance!")
        ...
        return message.reply(new MessageEmbed()
          ...
          .setFooter(message.author.tag, message.author.displayAvatarURL({dynamic: true}))
          .setTitle(`✅ You robbed \`${amount} 💸\` of \`${user.tag}\``)
