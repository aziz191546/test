...
module.exports = (client) => {
  ...
  client.on("messageReactionAdd", async (reaction, user) => {
    ...
    let ticket = client.setups.get(reaction.message.guild.id, "ticketsystem3");
    if (reaction.message.guild.id === ticket.guildid && reaction.message.id === ticket.messageid) {
      ...
      if (client.setups.get("TICKETS", "tickets3").includes(user.id)) {
        ...
        try{
          var channel = reaction.message.guild.channels.cache.get(client.setups.get(user.id, "ticketid3"))
          ...
          .setDescription(`<#${client.setups.get(user.id, "ticketid3")}>`));
        }catch{
      ...
      reaction.message.guild.channels.create(channelname.substr(0, 31), {
        ...
        client.setups.push("TICKETS", ch.id, "tickets3");
        ...
        .setAuthor(`Ticket for: ${user.tag}`, user.displayAvatarURL({dynamic: true}), "https://discord.gg/caxGNajAKJ")
