...
module.exports = (client) => {
  ...
  client.on("messageReactionAdd", async (reaction, user) => {
    ...
    if (reaction.message.guild.id === ticket.guildid && reaction.message.id === ticket.messageid) {
      ...
      if (client.setups.get("TICKETS", "tickets").includes(user.id)) {
        var es = client.settings.get(reaction.message.guild.id, "embed")
        try{
          var channel = reaction.message.guild.channels.cache.get(client.setups.get(user.id, "ticketid"))
          ...
          .setDescription(`<#${client.setups.get(user.id, "ticketid")}>`));
        }catch{
      ...
      reaction.message.guild.channels.create(channelname.substr(0, 31), {
        ...
        client.setups.push("TICKETS", ch.id, "tickets");
