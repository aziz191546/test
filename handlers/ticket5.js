...
module.exports = (client) => {
  ...
  client.on("messageReactionAdd", async (reaction, user) => {
    ...
    let ticket = client.setups.get(reaction.message.guild.id, "ticketsystem5");
    if (reaction.message.guild.id === ticket.guildid && reaction.message.id === ticket.messageid) {
      ...
      if (client.setups.get("TICKETS", "tickets5").includes(user.id)) {
        ...
        try{
          var channel = reaction.message.guild.channels.cache.get(client.setups.get(user.id, "ticketid5"))
          ...
          .setDescription(`<#${client.setups.get(user.id, "ticketid5")}>`));
        }catch{
      ...
      reaction.message.guild.channels.create(channelname.substr(0, 31), {
        ...
        client.setups.push("TICKETS", ch.id, "tickets5");
